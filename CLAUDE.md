...
- `scripts/ci-sweep-cancelled.sh [-y]`: finds cancelled ci runs on open PR
  ...
  The sweep refuses to rerun a run whose job ran within a minute of that
  job's own `timeout-minutes`, since rerunning a timeout only times out
  again and hides a budget problem; it prints the job and both numbers
  and exits 3, including in dry-run mode.
