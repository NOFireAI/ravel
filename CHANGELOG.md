...
- **A denied read on a catalog `idx/` object no longer degrades silently
  ...
  every tick, so this read as the postings tier or the reuse baseline
  ...
  `Err(LoadPostingsError::Store)` so the scrub tick can count it, and
  ...
  reuse is an optimization and not a correctness gate.
