...
SplitBrain: `publish_with_retry` panics on `PublishError::SplitBrain`; `handle_flush_join_result` resume_unwind -> shard actor dies -> `ShardUnavailable`, `IngestRouter::observe_shard_death`. Metrics: the death is counted once per incarnation, the actor is respawned with a fresh `writer_id` up to `MAX_SHARD_RESPAWNS` (3), and the next death past that budget condemns the shard (`IngestRouter::ready()` false, no further respawn). Logs and spans: `LogIngestRouter::mark_shard_dead` / `SpanIngestRouter::mark_shard_dead`, still a permanent per-shard stop (issue #1613). The model covers the death and the stopped flush, not the respawn.
