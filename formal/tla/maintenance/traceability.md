...
| ComputeLive | the once-per-cycle live-set snapshot; Rust does a LIST plus one GET per sibling, not atomic, a helper the model collapses to one step | `crates/ravel-fleet/src/worker_set.rs::live_set` | `crates/ravel-fleet/src/worker_set.rs::stale_sibling_is_excluded_at_three_h` | none |
...
| HeartbeatAndMemoNeverCas | heartbeat and memo are Overwrite, never CAS; two self-owned single PUTs | `crates/ravel-fleet/src/worker_set.rs::WorkerSet::write_heartbeat` | `crates/ravel-maintain/src/memo_snapshot.rs::write_memo_snapshot` | none |
