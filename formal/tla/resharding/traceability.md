...
| `RangeScanCount`, `EveryAdmittedWriteInScanSet` | The read set unions every generation live within slack of the query hour, so an admitted straggler stays listed; the range form takes the per-hour maximum over the query window | `crates/ravel-catalog/src/provisioning.rs::max_scan_count_over_range` | `crates/ravel-catalog/src/provisioning.rs::max_scan_range_decrease_inside_vs_outside_slack` | none |
...
| `AdmitFailClosed` fence | A refresh that fails past grace surfaces as a stale-view error rather than a stale route | `crates/ravel-ingest/src/error.rs::StaleProvisioningView` | `crates/ravel-ingest/src/generation.rs::cached_view_goes_stale_past_c`, `crates/ravel-ingest/src/error.rs::stale_provisioning_view_fails_closed_past_the_staleness_fence` | none |
...
| Reshard lead enforcement (CLI) | The reshard command refuses an activation inside `MIN_LEAD_HOURS` | `services/ravel-cli/src/provision.rs::reshard::MIN_LEAD_HOURS` | `services/ravel-cli/src/provision.rs::reshard_refuses_an_activation_lead_below_min_lead_hours` | none |
...
| `DoAdmit` write path | Admission routes a point through the active generation set and writes it | `crates/ravel-ingest/src/router.rs::write_points` | covered by the ravel-ingest router integration tests, `crates/ravel-ingest/src/router.rs::admitted_write_lands_on_the_shard_the_routed_count_selects` | none |
