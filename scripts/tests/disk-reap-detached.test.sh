...
source "${SCRIPT}"
# disk-reap.sh turns on errexit for its own main body. The cases below
# capture exit codes explicitly instead.
set +e
...
rebase_merge_onto_main() {
  local root="$1" sha="$2"
  git -C "${root}/repo" cherry-pick "${sha}" >/dev/null 2>&1
  git -C "${root}/repo" push -q origin main
  git -C "${root}/repo" fetch -q origin main
}
...
check_eq "merged PR: exit 0" "0" "${code}"
...
unset GH_FAKE_MODE
out="$(reap_dry "${root}")"; code=$?
check_eq "no merged PR: exit 0" "0" "${code}"
...
out="$(reap_dry "${root}")"; code=$?
unset GH_FAKE_MODE
check_eq "API down: exit 0" "0" "${code}"
...
# main must move first, or the replay reproduces the worktree's commit
# verbatim (same tree, same parent, same metadata: the same sha) and the
# ancestry check would answer before the fallback ever ran.
advance_main "${root}" "other.txt" "unrelated" "feat(scratch): an unrelated commit on main"
rebase_merge_onto_main "${root}" "${sha_pid}"
export GH_FAKE_MODE=down
out="$(reap_dry "${root}")"; code=$?
unset GH_FAKE_MODE
check_eq "patch-id: exit 0" "0" "${code}"
...
unset GH_FAKE_MODE
out="$(reap_dry "${root}")"; code=$?
check_eq "primary: exit 0" "0" "${code}"
