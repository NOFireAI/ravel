...
| `--logs-fetch-policy` | The logs read shape, resolved the way the server resolves its own flag of the same name. Unset means `cost-based`, which at the reference intra-region profile reads every log object whole in one covering GET, the shape a stock server has against a non-loopback endpoint (a stock server on a loopback `--s3-endpoint` derives `byte-minimal` instead, ADR-2014; pass that to match it). Passes taken before this flag existed range-read every object above 512 KiB per block, so their GET counts are not comparable with a default pass; `byte-minimal` reproduces that older shape. The report's `logs_fetch_policy` records the policy as requested, and `logs_block_range_threshold_effective` and `logs_request_cost_bytes_effective` record what it resolved to. |
| `--logs-block-range-threshold` | Object size in bytes above which a log object is range-read per block instead of read whole. Only meaningful under `byte-minimal`; `cost-based` at the reference profile overrides it with whole-object reads. Record it whenever a run sets it, or the pass cannot be reproduced. |
| `--logs-request-cost-bytes` | The explicit request-cost rate the routing weighs a GET at, in bytes. Unset means the rate the policy resolves at the profile; set, it wins over the policy-derived rate under every policy, `byte-minimal` included, so a run that combines the two must record both. Same reproduction rule as the threshold. |
