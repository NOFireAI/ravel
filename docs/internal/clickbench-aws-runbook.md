...
| `--logs-request-cost-bytes` | The explicit request-cost rate the cost-based routing weighs a GET at, in bytes. Unset means the rate the policy resolves at the profile. Same reproduction rule as the threshold. |
