...
`services/ravel-server/src/query.rs`'s `build_app_state` (the PromQL/HTTP
`AppState` builder) calls `with_memory_budget` with the process-wide
`MemoryBudget` (ADR-1170) on every engine it constructs, so a PromQL fetch's
reservation lands on the same shared instance `build_sql_state`'s
`SqlExecutor` reserves against (below). Two fetch paths still draw on their
own private default budget, unaffected by that wiring:
...
- **The SQL query path's own fetchers.** `build_sql_state`
  (`services/ravel-server/src/query.rs`) constructs its metrics, logs, and span
  fetchers with `with_get_limiter` but not `with_memory_budget`, so those
  fetchers still reserve against their own default unlimited budgets. The SQL
  path is not unbounded, though: `build_sql_state` installs the SAME
  process-wide `MemoryBudget` on the `SqlExecutor` itself via
  `SqlExecutor::with_process_memory_budget`, so a SQL query's per-tenant
  execution memory (`TenantMemoryAccountant`'s raw `try_reserve`/
  `reserve_unchecked` counter API) still competes with PromQL's fetch
  reservations for the same ceiling; only the SQL path's own fetch-layer GET
  reservations are exempt.

The default is `MemoryBudget::unlimited()`; a real process installs a finite
one at startup (see "Process-wide memory budget (ADR-1170)" below) and shares
the single `Arc` between `build_app_state` and `build_sql_state`, so the
reservations below refuse once that ceiling is reached rather than only
recording bytes.
...
them), and `ravel_memory_handoff_overlap_bytes` (bytes simultaneously held by
a live fetch `Reservation` and resident in the ADR-0046 read cache, because
the fetcher marked that reservation handed off once the cache took its own
independent copy of the same bytes; `0` when no fetch reservation is
...
`component="fetch"` is `MemoryBudget::fetch_reserved()`: bytes currently held
by a live `Reservation` opened through the RAII `reserve` path, which today
means a PromQL fetch issued by an engine `build_app_state` constructed (the
SQL path's own fetchers are not wired to this instance; see "Fetch-layer
memory reservations" above). `component="sql"` is
`MemoryBudget::sql_reserved()`, the total reserved minus `fetch_reserved()`,
i.e. every reservation taken through the raw `try_reserve`/
`reserve_unchecked` counter API (`SqlExecutor`'s per-tenant
`TenantMemoryAccountant`s). The two counters are disjoint by construction,
so they never double-count and always sum to the budget's whole reserved
total.
...
under S3's published guidance of about 5,500 GET/s per prefix. That guidance
is per prefix, which is what this bound is stated against: every request this
phase issues is bounded by the prefix it lands on, a commit record by its
`m/c/<shard>/<hour>/` prefix, a snapshot's parts by the one `snap/` directory
they share, its postings and column stats by their `idx/` directory, and a
LIST by the prefix it lists.
...
process never exceeds it, and no single prefix exceeds 128 however high it is
set. The ceiling scales with the queries the process admits rather than
staying at one prefix's worth, because N concurrent queries usually resolve
over N different shard-hours and would otherwise serialize behind a bound
sized for one.
...
committed block whose agreed row says `not measured in this run` is stating
that this repository's checked-in table proves reachability, not conformance.
Each row of the per-construct table keeps carrying the reached state, and a
construct that diverged from Prometheus in a folded-in run says so in its
evidence column.

The agreed row is scored against the constructs the run actually compared,
not against the whole surface. Its four counts are separate on purpose:
agreed over agreed plus diverged is the score, and the constructs no corpus
entry exercises are reported beside it as `not compared`. Summing that count
into the denominator would make a run in which Prometheus agreed with
everything it saw publish less than full agreement, and would leave a reader
unable to tell a construct that diverged from one that was never put to
Prometheus at all, which is the collapse the reached/agreed split exists to
remove. A run that compared nothing reads `no construct was compared`, since
`0/0` is neither 0% nor 100%.

The fourth count, `accepted divergence`, is out of both halves of the score.
A construct whose every exercising corpus entry is one an ADR already accepts
as diverging was never put to Prometheus for a match: a
...
`RAVEL_DIFFTEST_REPORT_OUT` names before it fails on a mismatch, and the job
feeds that file straight back into the conformance test through
`RAVEL_DIFFTEST_REPORT`, which prints the four counts in the job's log and
fails if the report is missing or malformed. The committed block stays the
Ravel-only rendering, because it is regenerated by a run with no Prometheus
binary in the loop and a differential rendering could not be reproduced by
anyone without the pinned binary: committing one would report the table as
stale in every environment that has none. So the agreed row above will keep
reading `not measured in this run` until something publishes a differential
block from a lane's archived report, which no lane does today.
