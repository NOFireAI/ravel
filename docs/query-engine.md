...
not against the whole surface. Its three counts are separate on purpose:
agreed over agreed plus diverged is the score, and the constructs no corpus
entry exercises are reported beside it as `not compared`. Summing that third
count into the denominator would make a run in which Prometheus agreed with
...
`RAVEL_DIFFTEST_REPORT`, which prints the three counts in the job's log and
