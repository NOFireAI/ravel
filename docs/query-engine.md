...
only a difftest lane that publishes a run report can move that row. Each row
of the per-construct table keeps carrying the reached state, and a construct
that diverged from Prometheus in a folded-in run says so in its evidence
column.
