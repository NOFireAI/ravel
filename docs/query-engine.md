...
dominated by the probes: q37-class drops from 11.7 GB to about 0.65 GB (the
~8,424 probes plus coalesced reads of about 144 surviving blocks), and q20
from 17.9 GB to about 4-4.5 GB (each of its ~6,592 surviving blocks read once,
never a whole-object re-read). The GET count is about two per segment in the
plan phase (probe + FIELD_DIR) plus the surviving-block and directory reads of
the segments that actually survive; the FIELD_DIR GET is the one request
resolving the numeric arms adds over the predicate-free path's single probe.
