...
| `--s3-allow-http` | `RAVEL_S3_ALLOW_HTTP` |  | Accept a plaintext `http://` `--s3-endpoint` whose host is not loopback. The S3 client's `allow_http` follows the endpoint's scheme, and a plaintext endpoint on the network carries every object this command writes and reads, plus the credentials signing those requests, in the clear; the command refuses that combination unless this flag says the operator meant it. A loopback `http://` endpoint (the local MinIO every development launcher here points at) needs no flag, and an `https://` endpoint is unaffected. Same flag, env var, and rule as ravel-server's |
