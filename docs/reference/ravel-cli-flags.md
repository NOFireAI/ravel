...
| `--max-ingest-lag` |  |  | The deployment's `ravel-server --max-ingest-lag` (humantime duration, e.g. `6h`). The catalog lists ingest-hour buckets from `--start` minus this value forward, so a server configured above the 2h default accepts records whose bucket a defaulted export would not list. Pass the server's own value when it differs |
