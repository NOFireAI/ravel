...
| `ravel_ingest_shards_condemned_total` | Shards that exhausted their respawn budget and were condemned, counted at most once per shard and never exceeding the shard count. Any nonzero value means the process reports `/readyz` 503 and is being replaced; alert on `> 0`. |
