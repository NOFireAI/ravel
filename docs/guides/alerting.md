...
across rule reloads. `state` is also mirrored onto the record's severity, so a
firing transition reads as an error-level record and a resolved one does not.
