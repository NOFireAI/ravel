...
against roughly $100-150/month of storage, and at a
...
3. **Flush cadence** (`--max-flush-delay`, `--max-flush-delay-idle`,
   `--min-flush-bytes` on `ravel-server`). The shipped
   default is 2s / 40s / 256KiB. This is the lever of last resort because, unlike the
   ...
   (raising one alone does little) and are checked at startup against a
...
## Measured effect of the cadence
...
--flush-delay-policy fixed`, comparing a 500ms/10s/64KiB cadence against the
shipped 2s/40s/256KiB cadence on the same workload. The in-memory store
isolates the ingest pipeline's own flush behavior from real object-store
latency. A request-count reduction is a property of flush cadence, not of
the backing store.

| | 500ms cadence | 2s cadence | Change |
|---|---|---|---|
...
The measured reduction (3.33x) is close to but under the steady-state
...
the direct, expected cost of a slower cadence. Durability itself is
unchanged. A strict-mode ack still only returns once the commit PUT that
makes the write durable has landed.

The full S3 cost-per-request benchmark (dollar cost against real
S3/MinIO, not PUT count) is tracked separately. Request-count
reduction is a property of the ingest pipeline alone and is measured here
without real object storage. A real dollar-cost figure needs a
...
- **Per-tenant caps** (200k active series/signal by default) mean a
  ...
  count.
- **The read-side request budget** is derived from shard
...
counts, byte counts, and timing differ (pinned by a
