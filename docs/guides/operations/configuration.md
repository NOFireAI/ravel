...
|---|---|---|---|
| `--fetch-concurrency` | derived: `max(8, 2 x cores)` | 32 | Legacy combined knob (ADR-1195): sets `--store-get-concurrency`, `--sql-partition-count`, and `--promql-fetch-fanout` together (source `legacy-flag`). Combining it with any of the three is a startup error naming both flags. |
...
| `--sql-partition-count` | derived: `max(8, 2 x cores)` | 32 | DataFusion `target_partitions` (`crates/ravel-sql/src/session.rs`). Host cores and query parallelism vs. per-partition overhead. |
