...
| `increase(ravel_provisioning_shard_count_mismatch_total[5m]) > 0`. | A tenant failed its provisioning check hard: an unreadable record (corrupt or a future format version), or pre-ADR data a lower `shard_count` would hide, caught on first touch or the maintenance loop (which skips that tenant's tick). A recorded count that merely differs from the live `--shards` default is no longer counted here; it is tolerated and tallied by `ravel_provisioning_shard_count_drift_total` instead. | The counter on `/metrics`. This does not take the process down: the one request fails with a typed error. | Read the durable record for that tenant. A corrupt or future-format record needs the record restored or the binary upgraded; a would-hide-data case needs `--shards` raised to cover the tenant's observed shards. Any nonzero increase is a real problem, not a rate to threshold. |
