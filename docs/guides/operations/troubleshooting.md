...
| Symptom | Likely cause | How to confirm | Corrective action |
|---|---|---|---|
| Storage keeps growing, retention deletes nothing, object counts per hour stay in the thousands. | There is no `--mode maintain` process. Ingest, query and alert evaluation all run without one; the scheduled catalog fold does not, so queries also pay listing cost for the whole unsealed span. | No process is running with `--mode maintain`. `ravel-cli maintain status --tenant <t> --signal <s> --shard <n> --hour <n>` reports a sealed, uncompacted bucket with a high L0 record count. | Deploy a maintain process. See [Maintenance](maintenance.md). |
