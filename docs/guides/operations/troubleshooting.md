...
| `increase(ravel_scrub_unreadable_total[1h]) > 0`. | The scrubber could not read an object or record, so it went unverified this rotation. Not corruption: the bytes were never read. `reason="access_denied"` is a key policy, a bucket policy, or a credential fault on the maintain process; `reason="permanent"` is any other store error retrying cannot clear, or a record whose bytes do not decode; `reason="retry_exhausted"` is a read that kept failing with a retryable error (throttled, timeout, transient) through six held ticks and once more, after which the scrub moved past it. | The counter, labeled by signal, `level` and `reason`, on the maintain process's `/metrics`, and the ERROR log line beside each count, which names the key. `ravel_scrub_marker_held_ticks` climbing toward 6 precedes a `retry_exhausted` count. | For `access_denied`, fix the maintain role's read grant or the object's encryption-key policy; it recurs every rotation until then. For `permanent`, read the named object with the same credentials to see the store's error; a record that does not decode needs the same investigation as a checksum mismatch. For `retry_exhausted`, check the store's health for that prefix; an error the backend reports as transient that never clears (an archived object, for example) lands here every rotation. |
