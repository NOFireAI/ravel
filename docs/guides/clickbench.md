...
Both write-window flags are passed together on purpose. Raising
`--pipeline-depth` alone over-subscribes a pipeline that cannot admit more than
`shards x max_inflight_flushes` concurrent writes, and the queued batches
exhaust their ack wait: a measured run at depth 16 against a per-shard cap of 1
aborted with `flush failed: timed out waiting for shard ack`. Raised together at
`4`, the same 100M-row corpus loaded in 1,519.75 s against 4,466.76 s at the
defaults, a 2.94x reduction, with the object count unchanged at 8,424 and cores
busy going from 2.33 to 8.58 of 16.
...
their acks, hiding the round-trip latency behind subsequent encode and I/O.

**The default is `1`, and the benchmark recipe above passes `4` explicitly.**
ADR-0807 keeps both write-window defaults at `1`: above `1`, a batch after a
failing one may already have committed in the background when the failure
happens, so its rows can become query-visible without being reported durable and
a resume re-ingests them. At `1` that cannot occur, because no later batch is in
flight. The speed-up is opt-in for exactly that reason.

Depth alone is not enough either — with one flush per shard the shards still
process their queued batches one PUT at a time — so a fast load must also raise
the per-shard concurrent-flush cap (`--max-inflight-flushes`, ADR-0067 decision
2), which lets each shard encode a later batch while an earlier one's PUT is in
flight. Raising the depth on its own over-subscribes the pipeline and produces
`flush failed: timed out waiting for shard ack`, measured at depth 16 against a
per-shard cap of 1. Object content is unchanged: commit
