...
| `--max-concurrent-gets <n>` | | `8` | Per-query bound on concurrent in-flight object-store GETs (issue #846), threaded into `EngineConfig::fetch_concurrency`: the fetch permit pool. Bounds concurrency at the store, not partition count in the plan (see `--scan-partitions`); a scan planned at more partitions than this only queues the surplus GETs on the pool. Size against the store's per-connection bandwidth and round-trip latency. Default is today's compiled-in value, so unset is byte-identical. See [guides/query.md](query.md#operator-configurable-budgets-server-flags). |
| `--scan-partitions <n>` | | (couples to `--max-concurrent-gets`) | Per-query SQL scan partition count (`target_partitions`), issue #846, threaded into `EngineConfig::scan_partitions`. Bounds plan parallelism, not store concurrency: every partition's fetches share `--max-concurrent-gets`'s permit pool. Unset couples it to the GET bound (the pre-split behavior), so leaving it off is byte-identical. |
| `--fetch-concurrency <n>` | | (deprecated) | Deprecated alias of `--max-concurrent-gets` (issue #846). Before the split this single flag set BOTH the in-flight GET bound and the SQL scan partition count, so no tuning result using it was attributable; it now controls only the GET bound it dominantly governed, and logs a deprecation warning. Conflicts with `--max-concurrent-gets`. Use `--max-concurrent-gets` and `--scan-partitions`. |
...
| `ravel-cli maintain compact-tenant --tenant <n> --signal <metrics\|logs\|spans> [--shards <n>] [--from-hour <n>] [--to-hour <n>] [--dry-run] [--max-flush-lifetime <duration>]` | `--shards` defaults to the tenant's provisioning record; `--from-hour` to each shard's oldest present hour; `--to-hour` to the current hour | Compacts every sealed bucket of one (tenant, signal), across shards `0..N`, replacing a hand-written per-(shard, hour) `compact-bucket` loop with one invocation that discovers the hours itself. Each shard's ingest hours are walked ascending and stop at the first unsealed one (every later hour is unsealed too). Prints one line per bucket with its outcome, then a summary: compacted / already / not_sealed / below_min / tombstoned counts, parts written, and wall time. Exits nonzero only on a compaction error; `NotSealed` is a reported outcome, not a failure. With no `--shards` and no provisioning record it refuses, naming the tenant; with both, the two must agree. `--dry-run` computes every plan but writes nothing. |
| `--max-flush-lifetime <duration>` (on `compact-bucket` and `compact-tenant`) | default `1h` | Overrides the compactor's `max_flush_lifetime` for this invocation. Humantime duration with the same grammar and unit as `ravel-server --gc-max-flush-lifetime` (`30m`, `1h5m`, `0s`). A bucket is sealed only once `now >= hour_end + max_flush_lifetime + clock_skew_allowance`, so a freshly finished load waits over an hour before any of its final hours can be compacted; lowering this seals them immediately. **Unsafe below the ingest path's real flush lifetime**: a bucket a writer is still flushing into can then be sealed and compacted, and that writer's later-published object is missed by the compaction. The default is the safe `1h`; use the override only for a tenant known quiescent, such as one whose bulk load has finished. `catalog fold` takes the same flag; see its row below for the fold's own version of this risk. |
...
| `ravel-cli catalog fold --tenant <name> [--shards <n>] [--signal <metrics\|logs\|spans>] [--max-flush-lifetime <duration>]` | `--shards` default `4`, `--signal` default `metrics` | One-shot catalog fold for one (tenant, signal): seals every eligible hour into a new snapshot part and CAS-advances that signal's HEAD. Prints the fold report (watermark before/after, the seal margin the watermark was computed with, buckets folded, entry count, request counts). A tenant's logs snapshot is a separate object from its metrics snapshot, so a logs or spans tenant is never folded unless `--signal` names it; without a snapshot, every query on that signal lists and reads every commit record in its window. Folding metrics on a logs-only tenant reports `entry_count 0` and publishes an empty metrics HEAD. This is the same operation the background fold task runs on a timer, which covers all three signals (metrics, logs, and spans) in every mode except `--mode maintain`, where it does not run at all. |
| `--max-flush-lifetime <duration>` (on `catalog fold`) | default `1h` | Overrides the fold's `max_flush_lifetime` for this invocation, the same grammar and unit as the compactor flag above (`30m`, `1h5m`, `0s`). An hour is sealed only once `now >= hour_end + max_flush_lifetime + clock_skew_allowance + fold_safety_margin` (1h20m by default), so a load that finished at 16:08Z has no snapshot covering its last hour until 18:21Z, and every query until then pays one commit-record GET per segment. `--max-flush-lifetime 0s` drops that to 20m. It says "no writer is still flushing", not "trust my clock": `clock_skew_allowance` and `fold_safety_margin` stay at their defaults and are not overridable here. **Unsafe under a live writer**: a commit record published into a bucket the fold already sealed is never picked up by a later incremental fold, which re-lists only hours after the watermark. Use it only for a tenant whose writer process has exited. The printed `seal_margin` line is the sum actually used, so a report shows what the override did. |
...
exited, nothing can publish into those hours any more, and waiting the margin
out only costs query time: until the fold covers them, every query pays one
commit-record GET per segment. `ravel-cli catalog fold --tenant <name>
--shards <n> --signal <signal> --max-flush-lifetime 0s` folds them at once.
It drops only the flush-lifetime term; `clock_skew_allowance` and
`fold_safety_margin` still apply (a 20m margin), so the hour currently being
written is still not sealed. The fold report's `seal_margin` line shows the
sum used. Do not use it while a writer for that tenant is live: a commit
record published into a bucket this fold already sealed is never picked up by
a later incremental fold, which re-lists only hours after the watermark, and
the repair is the HEAD-deletion rebuild below.
