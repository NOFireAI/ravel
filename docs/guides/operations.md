...
| `ravel-cli maintain compact-tenant --tenant <n> --signal <metrics\|logs\|spans> [--shards <n>] [--from-hour <n>] [--to-hour <n>] [--dry-run] [--max-flush-lifetime <duration>] [--bucket-concurrency <n>]` | `--shards` defaults to the tenant's provisioning record; `--from-hour` to each shard's oldest present hour; `--to-hour` to the current hour; `--bucket-concurrency` default `1` (today's fully sequential behavior byte-for-byte) | Compacts every sealed bucket of one (tenant, signal), across shards `0..N`, replacing a hand-written per-(shard, hour) `compact-bucket` loop with one invocation that discovers the hours itself. Each shard's ingest hours are walked ascending and stop at the first unsealed one (every later hour is unsealed too). Streams one line per bucket with its outcome in walk order as each bucket completes, then a summary: compacted / already / not_sealed / below_min / tombstoned counts, parts written, wall time, `failed:` count, and `bucket_concurrency:`. `--bucket-concurrency N` runs up to N buckets at once (refused at 0); a bucket whose compaction errors does NOT abort its siblings, so the walk completes the remaining buckets, prints each failed bucket's own `outcome=Failed error=...` line, and exits nonzero with an aggregate `BucketsFailed` naming how many failed and succeeded. A clean run exits zero; `NotSealed` is a reported outcome, not a failure. Each concurrent bucket gets a per-bucket share (`DEFAULT_MERGE_CURSOR_BUDGET_BYTES` 20 GiB / N, floor; N=1 unchanged) of the merge cursor budget, keeping the whole N-bucket memory envelope inside one box (ADR-0979); a merge that no longer fits its share fails closed with the typed `MergeCursorBudgetExceeded`. With no `--shards` and no provisioning record it refuses, naming the tenant; with both, the two must agree. `--dry-run` computes every plan but writes nothing. |
