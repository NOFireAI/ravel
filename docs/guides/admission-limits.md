...
| `ravel_ingest_resource_attrs_dropped_total` | counter | Metric resource attributes outside the allowlist, dropped rather than turned into labels. Not a rejection: the point that carried them was admitted. Normative description: [the observability guide](observability.md#resource-attributes-outside-the-allowlist). |
