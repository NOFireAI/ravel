...
|---|---|---|
| `--cache-max-bytes <n>` | fetcher cache derived: 25% of MemTotal; catalog byte cache derived: 5% of MemTotal (each `268435456`, 256 MiB, when memory cannot be read) | Maximum bytes the RAM tier holds. **Set**, it bounds **both** caches at that one value. **Unset**, the two derive independently: the fetcher cache at 25% of MemTotal (`8053063680` on the 30 GB reference host) and the catalog byte cache at 5% (`1610612736`), a smaller separate ceiling so the catalog cache does not double the fetcher's claim. Read once at startup; there is no live resize. |
...
(25% of MemTotal by default) plus the catalog byte cache (5%) plus the SQL
