...
can read the volume itself. A cache root you create yourself keeps the mode
you gave it; Ravel sets modes only on what it creates, which includes the
...
| `--cache-dir <path>` | none | Directory for the local-disk tier. Set, both the fetcher cache and the catalog byte cache gain a disk tier at this path, each bounded by its own resolved RAM ceiling: the fetcher cache's disk tier by `--cache-max-bytes` or its derived 25% share, the catalog byte cache's by its own resolved value (the derived 5% share, or `--cache-max-bytes` when that flag is set explicitly). There is no separate disk-tier capacity flag. Absent, the process has a RAM tier only. Bytes written here are not SSE-KMS encrypted (see "Two tiers" above); on Unix the entry files Ravel writes here are owner-read-write only and the directories it creates (this path and any missing ancestor of it included, when they do not exist yet) are owner-only, which is filesystem permissions rather than encryption; a cache root you create yourself keeps your own mode, and on startup Ravel narrows what an older build left wide beneath its own namespace. |
