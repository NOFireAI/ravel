...
from 11.7 GB to about 0.65-0.9 GB (about 8,424 probes plus coalesced reads of
about 144 surviving blocks), and q20 from 17.9 GB to about 4-4.7 GB (each of
its ~6,592 surviving blocks read once, no whole-object re-read).

The GET count does not fall in the same proportion, and is not meant to. The
object count is the floor: every relevant object still needs at least its
probe, so q37-class lands near 8,700 GETs — down from 19,690, but only just
under the 8,424 a full scan issues, about two to three per object. Bytes, not
requests, are what this amendment moves; a request-count reduction needs fewer
relevant objects (compaction, ADR-0102 decision 4's pruning), not better
intra-object pruning.

The 0.75 coverage crossover is unchanged; a numeric predicate whose survivors
still cover >= 75% of a segment reads that segment whole, exactly as before.
