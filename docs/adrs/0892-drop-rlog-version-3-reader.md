...
> The meaning of "first public release" throughout this ADR is settled by
> ADR-0531 (Accepted 2026-09-19): it is the **v1.0 release**, distinct from the
> software's first public release at 0.9.0. The question this ADR raises under
> "Rejected alternatives", of whether the N/N-1 window should begin before first
> release, is answered no: the window does not open before v1.0, and until then
> a format change may break backward compatibility outright. The two-version
> reader this ADR removes was an unretired predecessor, not that window opening
> early.
