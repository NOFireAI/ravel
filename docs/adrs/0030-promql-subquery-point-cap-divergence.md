...
Two `promql-difftest` subquery corpus mismatches, found once the harness
ran against the real pinned Prometheus binary
(subquery.txt's `error_subquery_grid_exceeds_the_point_cap` and
`error_nested_subquery_inner_grid_exceeds_the_point_cap`), are not
float-precision residue like the ADR-0025 cases; they are a deliberate
budget-enforcement divergence, and this ADR records the decision to accept
it.

## Context

Ravel enforces a resolution cap of 11,000 range-evaluation points per query
(`DEFAULT_MAX_RANGE_POINTS`, docs/query-engine.md "Budgets"). The
...
live binary at the time, that Prometheus would reject an equivalently
oversized subquery grid with a comparable error class. With the real binary
in the loop, that assumption is
...
- The cap itself is unaffected: it stays a documented, enforced budget
  (query-engine.md), and its correctness stays owned by
...
3. Remove the two entries from the differential corpus (mirroring the
   earlier move of non-differential aggregate cases to unit tests). Not
