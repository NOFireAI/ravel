...
different reason than the other three: the disjointness of `sys/`, `prov`,
and `catalog/` from every Maintain delete grant is enforced by
`no_delete_allow_reaches_the_disjoint_protected_keyspaces` in
`crates/ravel-commit/tests/iam_templates.rs`, which checks that no delete
`Allow` pattern in any template matches a witness key of those three
keyspaces. An audit object is keyed
