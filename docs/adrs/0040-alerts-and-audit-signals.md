...
Two workstreams both need a new kind of durable record that today's
`Signal` enum
(`crates/ravel-types/src/lib.rs`: `Metrics`, `Logs`, `Spans`, `Profiles`)
has no slot for:

- Unified alerting wants alert state itself stored as data, so a rule
  can read the alert history the same way it reads metrics or logs,
  including a rule that fires on other alerts ("alerts on alerts").
- Compliance custody wants an immutable audit log of query and admin
  activity, independent of any tenant-writable signal.
...
2. **No new segment format.** Both signals reuse RLOG v1
   ...
   - Audit: `actor` (tenant/API-key identity), `action` (`query`,
     ...
     `error`), plus whatever per-action detail fields the audit surface needs.
...
4. **Recursion guard for alerts-on-alerts.** A rule evaluation that
   ...
   hard circuit breaker on self-triggering chains, decided here rather
...
5. Audit records are written by `ravel-server` itself at defined
   ...
   by tenant-submitted rules; this ADR only reserves the signal, prefix,
   and format.
...
the alerting and audit surfaces test, not a format-level checksum concern.
...
- **One shared signal for both, e.g. reusing `Signal::Logs` with a
  ...
  Rejected: retention and future WORM policy must be configurable
  independently per signal - the existing per-tenant
...
- `ravel-catalog` and `ravel-maintain` need no new mechanism: `SegmentCodec`
  already dispatches on `bucket.signal` (ADR-0032); the alerting and
  audit work adds `Alerts`/`Audit` as new match arms.
- `ravel-sql` gets two new tables (`alerts`, `audit`) following the
  existing `logs` table's provider/pushdown pattern, not this ADR's work.
...
- Fuzz/property coverage for the RLOG decoder itself is unchanged and
  already exercises what both new signals rely on; the alerting and
  audit work adds attrs-convention tests (fold-by-`alert_id` correctness,
  generation circuit breaker), not new format fuzzing.
