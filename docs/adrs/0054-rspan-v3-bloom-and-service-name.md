...
Found while implementing the RSPAN inspector (it could not print a
bloom's offset/length/crc32c because none exists) and reconfirmed later:
the normative doc and the ADR disagree about whether v2 ever had a bloom,
and neither the bloom nor the column is reachable from any query path
today. `ravel-sql/src/spans_pushdown.rs` already extracts a
`service_name = <lit>` predicate from SQL; it has nothing to prune with.

A decision was needed: drop the bloom/column from the spec, or build
them. This ADR builds them.
...
body of work: attribute columns and span events. Claiming v3 for the
bloom and the column here collides with that reservation. Decision 2
below resolves the collision by renumbering decision 3's work to v4. No
code exists for decision 3 yet (verified: no branch, no stub), so the
renumbering has no code cost, only a documentation one.
...
retires v2 (ADR-0054); v4 retires v3 (this decision)". The follow-up
attribute-columns work gets the same v3→v4 substitution (doc-only edit,
no code exists yet to move).
...
them.** Rejected: both are wanted, and `service.name` equality is the entry
...
and has no branch or stub — building it as v4 while nothing v3-shaped
exists yet would leave v3 permanently unused. Renumbering the
not-yet-started work is strictly cheaper.
...
- Trailer version bump: `crates/ravel-rspan/src/footer.rs`'s `VERSION`
  ...
  treatment `rlog.rs`'s `OUTPUT_FORMAT_VERSION` got (cite
...
- ADR-0045 gets its decision-3 renumbering, in the same change that lands
  this ADR.
