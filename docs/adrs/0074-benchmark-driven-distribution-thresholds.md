...
crossover. An S3/MinIO multi-worker crossover measurement was pending.

The goal is "threshold autotuning," which leaves the meaning of
...
1. Measure the local-versus-distributed crossover on the reference host
   (`ci-16gb-fsn1-1`, the same-host baseline) against a
   realistic RSEG corpus over a real object store: loopback MinIO through
   `S3Store`, not `MemoryStore`. This matches ADR-0067's S3
...
2. Set the two default thresholds from the measured crossover: the corpus
   ...
   the benchmark log with a companion holding the raw output and the exact
   config.
...
3. "Autotuning" is an offline measured default, not a runtime-adaptive gate.
   ...
   autoscaling and observability work, which reads these measured
...
4. Re-validate the defaults when the reference host, or the fetch, decode, or
   ...
   carries the benchmark log's dagger convention until re-measured.
...
Reproducibility follows the benchmark rule already in force: the exact
...
committed alongside it. A run on any host other than the reference
...
flowchart LR
  ...
  defaults --> panel["benchmark panel\n+ raw report companion"]
...
## Measured result and resulting defaults
...
4, 16. Full panel and raw per-cell JSON are recorded with the "Distributed read
crossover (S3/MinIO, ci-16gb-fsn1-1)" panel.
...
Follow-ups tracked separately: warm-affinity cache measurement, a
corpus-span generator bug, the generator RAM cap, and a
...
- **A runtime-adaptive gate that learns from live query cost and latency.**
  ...
  that consumes these measured defaults rather than replacing them.
...
- **Measure on the dispatching workstation or any fleet executor.**
  The benchmark rule requires same-host comparison, and the stock executors
...
- `ravel-bench` gains a distributed-crossover benchmark that runs over
  `S3Store` (loopback MinIO) and reports the full matrix. A benchmark
  panel and a raw report companion document it.
...
- Later optimization work gates on this benchmark: order-insensitive
  aggregation pushdown, order-sensitive pushdown and its
  float-tolerance policy, and straggler hedging and rebalancing
