...
consumes it instead of issuing its own GET when supplied. The object crosses
the wire at most once per statement regardless of cache size or eviction. As
...
This carry does not bound peak memory to a fetch-concurrency scale. Every
relevant segment's plan result -- survivor count, stats, footer, and any
carried whole-object body -- is retained by `compute_plan_counts` until its
shared pass over every segment returns, because ADR-0102's flattened
block-striping assignment needs every segment's survivor count before any
partition can drain a block, not just the ones ahead of it. So a statement
that sends many relevant segments through the fallback at once
(a broad, unindexed text search across a large corpus) holds all of their
carried bytes simultaneously at that barrier: peak memory is bounded by the
tenant's admitted segment count (`ravel_query::SegmentAdmission`, 1024 by
default) times average object size, not by fetch concurrency times object
size. Reaching the tighter bound needs the barrier itself to release
progressively instead of all at once, a change to this ADR's partitioning
protocol that #835's fix does not make. `crates/ravel-sql/src/logs_scan.rs`'s
module doc ("Whole-object fallback carry, and its memory bound (issue #835)")
and `compute_plan_counts` report this peak via
`QueryAccounting::observe_intermediate_bytes` rather than leaving it
invisible, and
`crates/ravel-sql/tests/logs_selective_scan_amplification.rs`'s
`plan_barrier_peak_bytes_is_corpus_not_concurrency_bounded` pins it.
