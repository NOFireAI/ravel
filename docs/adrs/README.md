...
| [0815](0815-clustered-compaction-and-object-pruning.md) | Clustered compaction and object-level pruning: compaction sorts the merge by a time-leading clustering key and cuts parts on its boundaries so per-object bounds go narrow, reusing the existing event-bound exclusion plumbing (an additive catalog field only for a non-time override key), no segment-format bump | Proposed |
