...
| [1306](1306-request-budget-covers-unsealed-tail-and-fold-stall-window.md) | The derived per-query request budget covers the worst healthy unsealed tail plus the fold-stall alert window, so a stalled fold pages before any query is refused for its request budget. Amends ADR-0075 decision 1 | Accepted (2026-09-26) |
...
| [1702](1702-cpu-gate-and-isolated-liveness.md) | CPU-bound decode, encode and hash work leaves the tokio workers through two capped CPU gates (read and write) in front of `spawn_blocking`, and `/healthz` and `/readyz` are served from their own listener on a dedicated thread with a runtime heartbeat | Accepted (2026-09-26) |
...
| [2014](2014-loopback-store-defaults-to-byte-minimal-fetching.md) | Default to `byte-minimal` fetching when the S3 endpoint is loopback, where the cold path is disk-bound and requests are not billed; an explicit flag wins, concurrency and the cost profile are unchanged, and the policy's source is logged. Amends ADR-1196 | Proposed |
| [2023](2023-loopback-fetch-cache-share-and-independent-catalog-cache.md) | On a loopback store the fetch cache derives at a larger, measured share of the memory budget, so the ranged plan's block cache holds the working set under concurrency; `--cache-max-bytes` bounds the fetch cache only and the catalog cache is sized on its own. Amends ADR-1170 and ADR-2014 | Proposed |
