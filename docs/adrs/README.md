...
| [0102](0102-intra-segment-scan-partitioning-and-spill-policy.md) | Multi-core SQL execution: accept ADR-0094's parallel final aggregation in place, disable DataFusion's disk manager so budget exhaustion fails typed, a core-count scaling benchmark. Intra-segment scan partitioning deferred. | Accepted (partial) |
...
| [0106](0106-s3-instance-role-credentials.md) | S3 credentials from EC2 IAM instance roles: an explicit auth mode on S3Config so a server on EC2 needs no static keys | Claimed |
