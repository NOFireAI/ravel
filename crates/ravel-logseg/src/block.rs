...
//!
//! A block holds a run of records column by column. A column present in only
//! some rows of the block carries a presence bitmap page (`enc = Bitmap`)
//! immediately before its value page, so decode restores exact row alignment; a
//! column absent from every row of the block occupies zero bytes. A block's
//! pages are stored across its row group's column chunks and located through
//! PAGE_DIR (ADR-0699 decision 1), so this module encodes and decodes pages;
//! placing and checksum-verifying them belongs to the writer's `BlocksBuilder`
//! and to `crate::reader::decode_v4_block`.
...
/// -- so the membership test `decode_v4_block` runs once per page per block
/// has no HashDoS surface to resist, and paying
...
/// through to the per-page filter in `decode_v4_block`. Not the same type [`crate::ColumnSelection::resolve`]
...
///
/// The pages are handed back separately from any layout, because a block's
/// pages are spread across its row group's column chunks (ADR-0699 decision 1)
/// and the placement is the row group's decision, not this block's.
#[derive(Clone, Debug)]
pub struct BlockWriteOut {
    ...
    /// before `severity_text` (column 4). The block crc covers the pages in
...
///
/// A block decoded with a column filter holds only the filtered columns. A
/// column the filter excluded is indistinguishable
...
/// Decodes one block from pages the caller located through PAGE_DIR (ADR-0699
/// decision 2). A block has no header and no contiguous byte range: its pages
/// live in its row group's column chunks, so locating and checksum-verifying
/// them is the caller's job and this only decodes them.
///
...
pub fn read_block_pages(
    ...
    // A descriptor's `column_id` comes from PAGE_DIR, which is untrusted input,
    // and `ColMap` indexes a dense slot vector by it: the allocation follows the
    // largest id, not the column count, so one page claiming an id near
    // `u32::MAX` would resize that vector to billions of slots before any pool
    // limit could observe it.
    for d in descs {
        if u64::from(d.column_id) > MAX_COLUMN_ID {
            return Err(LogSegError::Corrupted(format!(
                "column_id {} over cap {MAX_COLUMN_ID}",
                d.column_id
            )));
        }
    }
...
mod tests {
    ...
    /// Decode a block through the page path the reader uses: slice each page's
    /// stored bytes out of [`BlockWriteOut::payload`] in descriptor order,
    /// decompress the ones the projection keeps, and hand the parallel arrays to
    /// [`read_block_pages`]. `columns == None` decodes every column.
    ///
    /// This is what `BlocksBuilder` and `crate::reader::decode_v4_block` do
    /// between them, minus the row-group placement. The placement moves where a
    /// page's bytes sit and what checksum covers them; it does not change what
    /// decodes out of them, which is what every test below is about.
    fn decode_out(
        out: &BlockWriteOut,
        plans: &[ColumnPlan],
        ...
        let mut page_bytes: Vec<Option<Vec<u8>>> = Vec::with_capacity(out.descs.len());
        let mut counters = PageCounters::default();
        let mut at = 0usize;
        for d in &out.descs {
            let end = at + d.len as usize;
            let stored = &out.payload[at..end];
            at = end;
            counters.bytes_fetched = counters.bytes_fetched.saturating_add(d.len);
            if wanted(d.column_id) {
                page_bytes.push(Some(read_page(stored, d, 1 << 30)?));
                counters.decoded += 1;
                counters.bytes_decoded = counters.bytes_decoded.saturating_add(d.len);
            } else {
    ...
    /// A page descriptor's `column_id` comes from PAGE_DIR, and `ColMap`
    /// indexes a dense slot vector by it, so the allocation follows the largest
    /// id rather than the column count. Without a cap a single page claiming a
    ...
    /// check in `read_block_pages` makes this test attempt a multi-gigabyte
    /// allocation instead of returning, which is how it was demonstrated
    /// failing.
    #[test]
    fn a_column_id_over_the_cap_is_rejected_before_it_allocates() {
        let descs = [PageDesc {
            column_id: u32::MAX, // wildly over cap
            enc: Enc::Plain,
            comp: 0,
            len: 0,
            uncomp_len: 0,
        }];
        let page_bytes = vec![Some(Vec::new())];

        match read_block_pages(1, &descs, &page_bytes, &[], PageCounters::default()) {
    ...
    fn roundtrip_with_mixed_presence() {
        ...
        let dec = decode_out(&out, &plans, None).expect("read");
    ...
    fn column_filter_decodes_only_the_named_columns() {
        ...
        let all = decode_out(&out, &plans, None).expect("read all");
        ...
        let projected = decode_out(&out, &plans, Some(&keep)).expect("read");
    ...
    fn page_byte_accounting_splits_fetched_and_decoded_by_filter() {
        ...
        // The page filter takes raw ids and adds nothing implicit, so this
        ...
        let single = |cid: u32| {
            ...
            decode_out(&out, &plans, Some(&keep))
                .expect("read")
                .page_bytes_decoded()
        };
    ...
    fn column_filter_membership_matches_expected_outcomes() {
        ...
        let decode = |filter: Option<&ColumnIdSet>| decode_out(&out, &plans, filter).expect("read");
        ...
        // Case 5: `None` is its own code path (the no-filter shortcut), not
        // "the set that happens to contain everything".
        let got = decode(None);
        assert_eq!(got.pages_decoded(), present.len());
        assert_eq!(got.pages_skipped(), 0);
    }
    ...
    fn dict_decoded_heap_bytes_reports_dictionary_footprint() {
        ...
        let dec = decode_out(&out, &plans, Some(&keep)).expect("read");
