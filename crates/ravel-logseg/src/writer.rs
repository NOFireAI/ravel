...
use crate::block::{BlockStrDict, ColumnPlan, ColumnarBlockInput, write_block, write_block_columnar};
            ...
            for p in &plans {
                ...
                for &g in block_rows {
                    if let Some(gid) = col_dict_ids[plan_idx][g] {
                        if seen.insert(gid) {
                            insert_text(&mut builder, p.column_id, &global_dict[plan_idx][gid as usize]);
                        }
                    }
                }
            }
