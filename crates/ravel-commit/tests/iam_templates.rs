...
/// True when an IAM glob admits EVERYTHING: it excludes no candidate at all, so
/// it constrains exactly as little as naming no pattern would. The test is
/// whether the pattern matches the empty string: `*` (and any all-`*` run such
/// as `**`) matches it, while a `?` -- which IAM resolves as exactly one
/// character -- does not, and any literal segment (`t/`, `sys/`) forces that
/// literal and so cannot match the empty candidate. Matching the empty string is
/// the precise, minimal reading of "admits everything": a `Resource` object-key
/// pattern the empty key matches covers every key in the bucket, and an
/// `s3:prefix` the empty prefix matches lets a caller pass the empty prefix and
/// list the whole bucket -- each the vacuous-value sibling of the empty-map and
/// empty-array holes this file already closes (issue #1346, F1). A narrower glob
/// (`t/*`, `sys/*`, `?`, `*x`) still excludes some candidate and is left alone;
/// rejecting a merely OVER-broad-but-non-vacuous pattern is a different guard.
...
/// closes, and no shipped template carries a `Deny` that withdraws a granted
/// KMS/Put capability (the only shipped `Deny` is `DenyDeleteProtected`, a
/// delete-only prohibition disjoint from every Allow). Closing it would require
/// each positive-capability guard to subtract the matching `Deny` before
/// asserting; that is a deliberate non-goal of this round. This is the axis the
...
///   nothing at all);
/// - a `Resource` object-key pattern that admits every key (a bucket-relative
///   `arn:aws:s3:::my-ravel-bucket/*`, which strips to `"*"`): shape-valid but
///   vacuous, scoping the object grant no more than naming no key would, the
///   resource-axis sibling of F1's `"*"` s3:prefix (issue #1346, F1 sweep);
...
/// - an `s3:prefix` VALUE that admits every key (a bare `"*"`, any all-`"*"`
///   run): shape-valid but vacuous, letting a caller list the whole bucket
///   exactly as an absent Condition would (issue #1346, F1).
    ...
    for resource in resources {
        match classify_resource(resource) {
            ResourceShape::ObjectKey(key) => {
                ...
                // sweep). A bounded prefix (`t/*`, `sys/*`) excludes keys outside
                // it and is unaffected.
                if glob_admits_everything(key) {
                    return Err(format!(
                        "{role}/{sid} (statement #{index}): Resource {resource:?} strips to the \
                         object-key pattern {key:?}, which matches every key under {BUCKET_ARN:?} \
                         (the empty key matches it) -- a full-bucket object grant constrains no \
                         more than naming no key scope at all; it must name a bounded key prefix \
                         (issue #1346, F1 sweep)"
                    ));
                }
                saw_object = true;
...
/// constrain. A value that `glob_admits_everything` accepts -- a bare `"*"`, any
/// all-`"*"` run -- passes every shape check yet lets a caller list the whole
/// bucket, constraining no more than an absent Condition. It is rejected for the
/// same reason the empty block is, one level down in the value (issue #1346, F1).
                    ...
                    if glob_admits_everything(prefix) {
                        return Err(format!(
                            "{role}/{sid} (statement #{index}): Condition {operator:?}.{cond_key:?} \
                             value {prefix:?} admits every key (the empty prefix matches it), so it \
                             constrains no more than an absent Condition -- an s3:prefix must \
                             exclude some key outside its listed prefixes (issue #1346, F1)"
                        ));
                    }
                }
            }
