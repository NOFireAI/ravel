...
mod tests {
    ...
    fn all_four_custom_buckets_rules_are_enforced_on_both_surfaces() {
        ...
        use opentelemetry_proto::tonic::metrics::v1::{
            ...
            ResourceMetrics, ScopeMetrics, exponential_histogram_data_point::Buckets,
        ...
        // A NaN `zero_count` goes with them: nothing screens `zero_count`
        // before this rule (unlike `zero_threshold`, which
        // `zero_threshold_is_admissible` has already narrowed to the finite
        // non-negatives), so a magnitude test spelled `> 0.0` would admit it
        // here, every comparison with NaN being false.
        for zero_count in [5e-324f64, f64::NAN] {
            let mut h = custom_buckets_histogram(1_000);
            h.count = Some(ResolvedCount::Float(14.0));
            h.positive_counts = vec![2.0, 5.0, 6.0];
            h.positive_deltas = vec![];
        ...
        // The predicate both zero-side rules rest on, asserted directly so
        // the evidence does not depend on which checks ran before it. Under
        // the call order in `build_histogram_value` an earlier gate has
        // already refused a NaN or infinite `zero_threshold`, so a magnitude
        // test would agree with this on that field by accident.
        for v in [0.0f64, -0.0f64] {
            assert!(is_zero_f64(v), "{v:?}");
        }
        for v in [
            5e-324f64,
            f64::MIN_POSITIVE,
                    ...
                    resource_metrics: vec![ResourceMetrics {
                        scope_metrics: vec![ScopeMetrics {
                            metrics: vec![Metric {
                                ...
                                data: Some(MetricData::ExponentialHistogram(
