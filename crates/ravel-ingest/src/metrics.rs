...
pub struct IngestMetrics {
    ...
    /// passes (ADR-1307 finding F1). A graceful drain re-buffers a
    /// clock-refused flush and retries it in the same call; this counts the
    /// residue that survived every pass, which is a lost acknowledged
    /// buffered-mode write on a graceful teardown (a pathological clock that
    /// steps back on every reading). Nonzero is a durability defect, logged at
    /// ERROR beside this bump so the residue is never silent.
...
impl IngestMetrics {
    ...
    /// finding F1). Called once per `flush_all` that leaves a residue, with the
    /// residual tenant count.
