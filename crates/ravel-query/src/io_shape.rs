...
//!   `buffer_unordered(fetch_concurrency)` over distinct matcher plans
//!   (`distinct_plans_by_matcher`), each running its own
//!   `buffer_unordered(fetch_concurrency)` over that plan's segments. Those
//!   inner streams do not get independent budgets -- every GET from every
//!   plan passes through the one semaphore `SegmentFetcher::get_semaphore`
//!   (`fetcher.rs`), sized `SegmentFetcher::max_concurrent_gets`
//!   (`DEFAULT_MAX_CONCURRENT_GETS` = 16 unless overridden). So the binding
//!   concurrency is `min(service_fetch_multiplier * fetch_concurrency,
//!   shared_get_permits)`, and the work is `segment_count *
//!   service_fetch_multiplier` GETs: 64 segments, `fetch_concurrency` 8, 2
...
//!   the un-multiplied `fetch_concurrency` alone double-counts the plan
//!   fan-out. A single-plan query (`service_fetch_multiplier == 1`) reduces
//!   to `ceil(segment_count / fetch_concurrency)` unchanged, since
//!   `min(fetch_concurrency, shared_get_permits)` is `fetch_concurrency`
