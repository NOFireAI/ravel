...
pub struct FederationOutcome {
    ...
    /// `Vec` per remote (ADR-0096 decision 3 step 4). The engine merges these
    /// into its histogram pool the same way it merges the local histogram runs,
    /// so a federated histogram query is the union of what each cluster computes
    /// locally -- never silently dropped.
