...
//! extracts its longest token-bounded mandatory literal run, or nothing when
//! no such run exists (`.*word.*`, any `+`-quantified pattern) -- token
...
pub struct LogSeriesOutput {
    ...
    /// The reader's own per-object block counters (`ScanStats`), summed over
    /// every segment this call actually opened a scan for. Zero for a
