...
impl QueryEngine {
        ...
        Fut: std::future::Future<Output = Result<(T, FetchStats), QueryError>>,
    {
        ...
        let shared_get_permits = self.fetcher.max_concurrent_gets() as u64;
...
/// one `buffer_unordered(fetch_concurrency)` over distinct plans
...
/// from every plan passes through the same shared
/// `SegmentFetcher::get_semaphore` (`fetcher.rs`), whose capacity is
/// `shared_get_permits` here (`SegmentFetcher::max_concurrent_gets`,
/// `DEFAULT_MAX_CONCURRENT_GETS` = 16 unless overridden -- `QueryEngine::new`
/// calls `with_max_concurrent_gets` on the log fetcher only, never on
/// `self.fetcher`, so the metrics fetcher runs at that default). So the real
/// binding concurrency is `min(service_fetch_multiplier * fetch_concurrency,
/// shared_get_permits)`: the outer `buffer_unordered` can never have more
/// than `service_fetch_multiplier * fetch_concurrency` plan/segment fetches
/// in flight at once even with an unbounded semaphore, and the shared
/// semaphore can never let more than `shared_get_permits` GETs run at once
...
/// fetch_concurrency, shared_get_permits))`, not `service_batches(segments *
/// service_fetch_multiplier, fetch_concurrency)`: dividing the
/// plan-multiplied numerator by the un-multiplied `fetch_concurrency` alone
...
/// `fetch_concurrency` 8, `shared_get_permits` 16: 128 GETs at binding
/// concurrency `min(16, 16) = 16` is 8 batches, not the 16 the unscaled
/// divisor reports).
///
/// A single-plan query (`service_fetch_multiplier == 1`) is unaffected:
/// `min(1 * fetch_concurrency, shared_get_permits)` is `fetch_concurrency`
/// whenever the semaphore pool is at least as large as one plan's own
/// `buffer_unordered` bound, which every configured pool in this crate is.
///
...
mod prefetch_tests {
    ...
    /// concurrency (`min(distinct_matcher_sets * fetch_concurrency,
    /// shared_get_permits)`), not by `fetch_concurrency` alone. Two distinct
    ...
    /// resolves all 64 published segments), the default `fetch_concurrency`
    /// (8), and the metrics fetcher's default shared `SegmentFetcher`
    /// permits (`DEFAULT_MAX_CONCURRENT_GETS` = 16, since `QueryEngine::new`
    /// only overrides the log fetcher's semaphore): 128 GETs (64 segments *
    /// 2 distinct matcher sets) at binding concurrency `min(2 * 8, 16) ==
    /// 16` is 8 batches. Before the fix, `io_shape_for_resolve` divided the
    /// plan-multiplied numerator by `fetch_concurrency` alone --
    /// `ceil(64 * 2 / 8) == 16`, double the real figure. Flip
    /// `binding_concurrency` in `io_shape_for_resolve` back to plain
    /// `concurrency` to watch the assertion below fail: it would then read
    /// 16, not 8.
    #[tokio::test]
    ...
    /// (`distinct_matcher_sets * fetch_concurrency`), not the shared
    /// semaphore, is the binding constraint, so the pre-fix and post-fix
    /// formulas disagree on a case where the semaphore was never close to
    /// full. `fetch_concurrency` is lowered to 4 (`EngineConfig`), so 2
    /// distinct matcher sets give a plan fan-out bound of `2 * 4 == 8`,
    /// strictly under the fetcher's default 16 shared permits: `min(8, 16)
    /// == 8` is bound by the plan fan-out, not the semaphore. 20 segments * 2
    /// matcher sets = 40 GETs at that binding concurrency is `ceil(40 / 8)
    /// == 5`. The pre-fix formula (dividing by `fetch_concurrency` alone,
    /// ignoring the multiplier entirely) would have read `ceil(20 * 2 / 4)
    /// == 10`, twice the correct figure -- a different wrong answer than the
    /// worked acceptance case's, because here the multiplier's own
    /// contribution to the divisor was never capped by the semaphore.
    #[tokio::test]
    async fn service_batches_binds_on_plan_fanout_when_semaphore_has_headroom() {
        ...
        let eng = engine_with_config(store, config);
        let plans = vec![
            SelectorPlan {
                range_ns: WIDE_RANGE_NS,
        ...
        assert_eq!(
            ...
            "plan fan-out binds here, not the semaphore: min(2 distinct \
             matcher sets * 4 fetch_concurrency, 16 shared permits) == 8, so \
             20 segments * 2 matcher sets = 40 GETs is ceil(40 / 8) == 5"
        );
    }
