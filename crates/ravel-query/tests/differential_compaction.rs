...
//! input's commit record at compaction). That per-run emission is exactly
//! what makes a query over the L1 part reproduce the pre-compaction candidate
//! set.
//!
...
//! per-run L1 emission and the resolver's include-parts/exclude-inputs
...
use ravel_maintain::{
    Bucket, CompactionOutcome, CompactorConfig, FixedClock, PublishOutcome, compact_bucket,
};
...
/// Wall-clock read time for `resolve`: mid-hour. Resolution lists the bucket
/// by shape and never filters records by `created_unix_ns <= now`, so this is
/// independent of the (later) compaction clock.
fn now_ns() -> i64 {
    hour_start() + 30 * 60_000_000_000
}

/// Wall-clock time for the compactor: well past the hour's seal margin
/// (`end_ns + max_flush_lifetime + clock_skew`, ~2h5m after `hour_start`), so
/// `Bucket::is_sealed` holds and `compact_bucket` proceeds.
fn compaction_now_ns() -> i64 {
    hour_start() + 3 * NS_PER_HOUR
}
...
/// The object-store keys of one flushed L0, so a test can sweep it after
/// compaction to model the post-sweep steady state.
#[derive(Debug, Clone)]
struct L0Handle {
    commit_key: String,
    data_key: String,
...
/// Write one real RSEG v5 L0 segment plus its commit record into `HOUR`.
async fn build_l0(store: &dyn ObjectStoreBackend, spec: &WriterSpec) -> L0Handle {
    ...
    let commit_key = commit_key_for_record(&record).expect("commit key");
    store
        .put(
            &commit_key,
    ...
    L0Handle {
        commit_key,
        data_key: record.object_key,
    }
}

/// Run P4's real compactor over the whole `HOUR` bucket, publishing the
/// compaction record and its L1 parts. Asserts it actually compacted (the
/// bucket is sealed and above `min_compaction_inputs`).
async fn compact(store: &dyn ObjectStoreBackend) {
    let bucket = Bucket::new(tenant_hash(), Signal::Metrics, SHARD, HOUR);
    let clock = FixedClock::new(compaction_now_ns());
    let cfg = CompactorConfig::default();
    let outcome = compact_bucket(store, &clock, &cfg, &bucket)
        .await
        .expect("compact_bucket");
    match outcome {
        CompactionOutcome::Compacted { parts, publish } => {
            assert!(parts >= 1, "compaction produced at least one L1 part");
            assert_eq!(publish, PublishOutcome::Published);
        }
        other => panic!("expected the bucket to compact, got {other:?}"),
    }
}
...
/// input handles (for sweeping).
async fn seed_store(specs: &[WriterSpec]) -> (Arc<MemoryStore>, Vec<L0Handle>) {
...
async fn assert_differential(specs: &[WriterSpec]) {
    ...
    // The resolver excludes the listed inputs, so this must still match.
    let (mixed_store, _) = seed_store(specs).await;
    compact(mixed_store.as_ref()).await;
    ...
    compact(l1_store.as_ref()).await;
    for handle in &l1_inputs {
        l1_store
            .delete(&handle.commit_key)
            .await
...
fn writer_strategy() -> impl Strategy<Value = WriterSpec> {
    let samples = prop::collection::vec((0u8..3u8, 0i64..6i64, value_bits()), 1..6).prop_map(
        |raw: Vec<(u8, i64, u64)>| {
            // Dedup (series, ts) within one writer: an L0 flush has one run
            // per series with strictly ordered timestamps, so keep the first
            // bits seen for each (series, ts).
...
// --- Mixed L0/L1 snapshot end-to-end ---

/// Write one real RSEG v5 L0 with a single scalar sample for `series idx`.
async fn build_single(
    ...
    build_l0(
        ...
        &WriterSpec {
            ...
            seq,
            samples: vec![(idx, ts_off, value.to_bits())],
        },
    )
    .await
}
...
#[tokio::test]
async fn mixed_l0_l1_snapshot_end_to_end() {
    // Two L0s over one series (ts 10 and 20) plus one L0 over a second series,
    // all in HOUR. compact_bucket rewrites the whole sealed bucket into L1
    // parts; the resolver then serves a snapshot of L1 parts only. This
    // exercises the full fetch-and-decode path over a real compacted bucket:
    // the per-run L1 emission must recover every input sample with its exact
    // bit pattern.
    let store = Arc::new(MemoryStore::new());
    build_single(store.as_ref(), 1, 0, 10, 5.0).await;
    build_single(store.as_ref(), 2, 0, 20, 6.0).await;
    build_single(store.as_ref(), 3, 1, 10, f64::NAN).await;
    compact(store.as_ref()).await;
    ...
    let snapshot = catalog
        ...
        .await
        .expect("resolve");
    // The whole bucket compacted: every surviving segment is an L1 part, and
    // no L0 input survives (all are named by the compaction record).
    assert!(
        !snapshot.segments.is_empty(),
        "at least one L1 part must resolve"
    );
    assert!(
        snapshot
            .segments
        ...
        "every resolved segment is an L1 part"
    );

    let merged = dedup(&fetch_all(store).await);
    // Two series present.
    assert_eq!(merged.len(), 2);
    // The NaN series round-trips its exact bit pattern.
    let (_m, nan_labels) = labels_for(1);
    let nan_sid = SeriesId::compute(&TenantId::new("t"), "m1", &nan_labels).unwrap();
    let nan_samples = &merged[&nan_sid.0];
    assert_eq!(nan_samples.len(), 1);
    assert_eq!(nan_samples[0].1, f64::NAN.to_bits());
    // The multi-run series carries both compacted runs' samples (ts 10 and 20).
    let (_m0, s0_labels) = labels_for(0);
    let s0_sid = SeriesId::compute(&TenantId::new("t"), "m0", &s0_labels).unwrap();
    let s0_samples: Vec<i64> = merged[&s0_sid.0].iter().map(|(ts, _)| *ts).collect();
    assert_eq!(s0_samples, vec![hour_start() + 10, hour_start() + 20]);
}
...
async fn l1_footer_identity_verified_against_record() {
    let store = Arc::new(MemoryStore::new());
    build_single(store.as_ref(), 1, 0, 10, 1.0).await;
    build_single(store.as_ref(), 2, 0, 20, 2.0).await;
    compact(store.as_ref()).await;
    ...
    // is caught by footer identity verification: the object's v5 footer no
