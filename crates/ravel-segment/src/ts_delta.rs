...
//! delta from the run minimum, then zigzag-varint deltas between consecutive
//! timestamps (docs/segment-format.md, ADR-0092 decision 4). Shared by the
//! writer (encode) and reader (decode) so both sides can never disagree about
//! the byte grammar. Because the writer sorts a run ascending, its first
//! timestamp IS the run minimum, so the stored first delta is a single zero
//! byte instead of an absolute ~9-byte varint from zero.

use crate::error::SegmentError;
use crate::varint::{read_zigzag_varint, write_zigzag_varint};

/// Encodes `timestamps` as delta-varint bytes, the first as a delta from
/// `run_min_ts_ns`. Test-only convenience; production encoding goes through
/// [`encode_ts_deltas_into`] with a reused buffer.
#[cfg(test)]
pub fn encode_ts_deltas(timestamps: &[i64], run_min_ts_ns: i64) -> Option<Vec<u8>> {
    let mut out = Vec::with_capacity(timestamps.len() * 2);
    encode_ts_deltas_into(&mut out, timestamps, run_min_ts_ns)?;
    Some(out)
}

/// Appends the delta-varint encoding of `timestamps` to `out` without
/// allocating, so the writer can reuse one scratch buffer across series. The
/// first timestamp is stored as a zigzag-varint delta from `run_min_ts_ns`
/// (the run's minimum event timestamp, which the reader reconstructs from the
/// catalog and hands back to the decoder), so a sorted run whose first
/// timestamp equals the run minimum stores a single zero byte there.
pub fn encode_ts_deltas_into(
    out: &mut Vec<u8>,
    timestamps: &[i64],
    run_min_ts_ns: i64,
) -> Option<()> {
...
/// preserved; on error the appended tail is unspecified). The first
/// timestamp reconstructs as `min_ts_ns + first_delta`, matching the
/// encoder's delta-from-run-minimum grammar (ADR-0092 decision 4). Each
/// decoded value is validated against `[min_ts_ns, max_ts_ns]`
/// (docs/segment-format.md). Rejects trailing bytes past the declared count.
/// Appending (rather than clearing first) lets a caller concatenate several
/// runs' pages into one buffer, which the query fetcher's L0 branch relies on
/// to emit one unit per series with every run's samples in on-disk order; a
/// caller that wants a fresh decode clears the buffer itself.
...
mod tests {
    ...
    #[test]
    fn first_timestamp_encodes_as_zero_delta_from_run_min() {
        // The writer encodes a run's first timestamp as a delta from the run
        // minimum (docs/segment-format.md, ADR-0092 decision 4). Because the
        // writer sorts ascending, the first timestamp IS the run minimum, so
        // the stored first delta is a single zero byte instead of a ~9-byte
        // absolute varint at current nanosecond epochs.
        let run_min = 1_650_000_000_000_000_000i64;
        let ts = [run_min, run_min + 15, run_min + 40];
        let encoded = encode_ts_deltas(&ts, run_min).expect("no overflow");
        assert_eq!(
            encoded[0], 0x00,
            "first delta from the run minimum is zigzag(0) = one zero byte"
        );
        // zigzag(15)=30=0x1e; zigzag(25)=50=0x32.
        assert_eq!(encoded, vec![0x00, 0x1e, 0x32]);
        let decoded = decode_ts_deltas(&encoded, 3, run_min, run_min + 40).expect("decodes");
        assert_eq!(decoded, ts);
    }

    #[test]
    fn hand_vector_ascending_with_duplicate_and_gap() {
        // ts = [10, 10, 25, 100], run_min = 10; deltas = [0, 0, 15, 75].
        // zigzag(0)=0x00; zigzag(0)=0x00; zigzag(15)=30=0x1e;
        // zigzag(75)=150 -> varint [0x96, 0x01] (150 = 0b1001_0110:
        // low7=0x16 with continuation -> 0x96, remaining=1 -> 0x01).
        let ts = [10i64, 10, 25, 100];
        let encoded = encode_ts_deltas(&ts, 10).expect("no overflow");
        assert_eq!(encoded, vec![0x00, 0x00, 0x1e, 0x96, 0x01]);
    ...
    fn hand_vector_decode_tolerates_decreasing_deltas() {
        ...
        // writer always pre-sorts ascending). ts=[5,3], run_min=3;
        // delta0=5-3=2 (zigzag 4=0x04); delta1=3-5=-2 (zigzag 3=0x03).
        let encoded = [0x04, 0x03];
    ...
    fn empty_sequence_roundtrips() {
        let encoded = encode_ts_deltas(&[], 0).expect("no overflow");
    ...
    fn encode_rejects_delta_overflow() {
        // i64::MAX - i64::MIN overflows i64 subtraction (the second delta).
        assert_eq!(encode_ts_deltas(&[i64::MIN, i64::MAX], i64::MIN), None);
    }

    #[test]
    fn encode_rejects_first_delta_overflow() {
        // The first delta is ts[0] - run_min; a run_min far above ts[0]
        // overflows and is rejected rather than wrapping.
        assert_eq!(encode_ts_deltas(&[i64::MIN], i64::MAX), None);
    }

    #[test]
    fn decode_rejects_accumulation_overflow() {
        // With base min_ts_ns = 0: accum0 = i64::MAX, then + i64::MAX overflows.
        let mut bytes = Vec::new();
        write_zigzag_varint(&mut bytes, i64::MAX);
        write_zigzag_varint(&mut bytes, i64::MAX);
        assert_eq!(
            decode_ts_deltas(&bytes, 2, 0, i64::MAX),
    ...
    fn decode_rejects_out_of_bounds_timestamp() {
        let ts = [1i64, 2, 3];
        let encoded = encode_ts_deltas(&ts, 1).expect("no overflow");
        // Declare bounds that exclude 3 (base min stays 1 for symmetry).
        assert_eq!(
    ...
    fn decode_rejects_trailing_bytes() {
        let ts = [1i64, 2, 3];
        let mut encoded = encode_ts_deltas(&ts, 1).expect("no overflow");
...
mod fuzz_mutation {
    ...
    proptest! {
        ...
        fn roundtrips_ascending_timestamps(
            ...
            // Ascending, so the minimum is the first element (== base).
            let run_min = ts.first().copied().unwrap_or(0);
            let encoded =
                encode_ts_deltas(&ts, run_min).expect("no overflow for bounded deltas");
        ...
        fn seed_mutation_never_panics(
            ...
            let run_min = ts.first().copied().unwrap_or(0);
            let encoded =
                encode_ts_deltas(&ts, run_min).expect("bounded deltas never overflow");
