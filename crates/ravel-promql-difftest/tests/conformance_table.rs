...
/// Environment variable naming a JSON [`RunReport`] from the differential lane
/// (`RAVEL_DIFFTEST=1`, the pinned Prometheus binary in the loop). It is the
/// only thing that populates the agreed dimension: with it unset, every run
/// here is Ravel-only and the block says so.
const REPORT_ENV: &str = "RAVEL_DIFFTEST_REPORT";
...
/// The acceptance test for issue #1698. The committed table is regenerated by
/// this crate with no Prometheus binary in the loop, so the block it produces
/// must say which run it is: a reader takes a lone score row for agreement
/// with Prometheus, and this run measures no such thing.
///
/// The report is built with `None` rather than reading [`REPORT_ENV`], so the
/// assertion is about the renderer and not about the environment the test
/// happens to run in.
#[tokio::test(flavor = "multi_thread")]
async fn generated_block_names_the_run_ravel_only_when_difftest_is_unset() {
    ...
    assert_eq!(
        report.agreed_count(),
        None,
        "a Ravel-only run measures no agreement, so there is no agreed count"
    );
    assert!(
        block.contains(RAVEL_ONLY_RUN_LABEL),
        "the block does not name the run Ravel-only:\n{block}"
    );
    assert!(
        !block.contains(DIFFERENTIAL_RUN_LABEL),
        "a Ravel-only run must not claim a differential one:\n{block}"
    );
    // The two score rows are separate claims: the reached one is a number, the
    // agreed one is not measured here and says so rather than printing 0 or a
    // full-marks figure.
    ...
    assert!(
        ...
        "the block carries no reached score row:\n{block}"
    );
    assert!(
        block.contains(&format!(
            "| {AGREED_ROW_LABEL} | **{AGREED_NOT_MEASURED}** |"
        )),
        "the agreed score row does not read '{AGREED_NOT_MEASURED}':\n{block}"
    );
    assert!(
        !block.contains(&format!("| {AGREED_ROW_LABEL} | **0/")),
        "an unmeasured agreement is not zero:\n{block}"
    );
    // The old single headline, which is what a reader was taking as agreement.
    assert!(
        !block.contains("| **score** ("),
        "the block still carries the undifferentiated score row:\n{block}"
    );
}

/// The agreed dimension against a real run of the real corpus: a synthetic
/// run report failing exactly one named corpus entry. The entry is one the
/// corpus really carries, so the constructs it demotes are the constructs it
/// exercises, and the agreed count is pinned to an exact number rather than a
/// band.
#[tokio::test(flavor = "multi_thread")]
#[allow(clippy::expect_used)]
async fn a_supplied_run_report_populates_the_agreed_score() {
    let parsed = parsed_corpus();
    let entries: usize = parsed.iter().map(|(_, e)| e.len()).sum();

    let clean = build_report_with(Some(&RunReport {
        total: entries,
        failures: Vec::new(),
    }))
    .await;
    ...
    let diverged = build_report_with(Some(&RunReport {
        total: entries,
    ...
    .await;
    assert_eq!(
        diverged.agreed_count(),
        Some(agreed_clean - demoted.len()),
        "exactly the constructs '{failing}' exercises must leave the agreed set"
    );

    let block = diverged.to_markdown();
    assert!(
        block.contains(DIFFERENTIAL_RUN_LABEL),
        "block was:\n{block}"
    );
...
async fn the_unclassified_bucket_is_reported_not_hidden() {
    ...
    eprintln!(
        "PromQL conformance: {} supported, {} intentionally rejected, {} \
         accepted divergence, {} unclassified (reached {} of {} = {}%, agreed \
         with Prometheus: {})",
        counts.supported,
...
/// Builds the report from a real run, folding in a differential run report
/// when [`REPORT_ENV`] names one.
#[allow(clippy::expect_used)]
async fn build_report() -> ConformanceReport {
    let differential = env::var(REPORT_ENV).ok().map(|path| {
        let path = PathBuf::from(path);
        load_run_report(&path).unwrap_or_else(|e| panic!("{REPORT_ENV}={}: {e}", path.display()))
    });
    build_report_with(differential.as_ref()).await
}

/// Reads a JSON [`RunReport`] written by the differential lane.
///
/// Strict on every field it reads: a report whose failures cannot be read
/// would fold in as a clean run and publish agreement nobody measured, so a
/// malformed file is an error rather than an empty failure list. `total` is
/// what proves the run covered the corpus, so it is required too.
fn load_run_report(path: &Path) -> Result<RunReport, String> {
    let text = std::fs::read_to_string(path).map_err(|e| format!("reading the report: {e}"))?;
    let json: Json = serde_json::from_str(&text).map_err(|e| format!("parsing the report: {e}"))?;
    let total =
        json.get("total")
            .and_then(Json::as_u64)
            .ok_or_else(|| "the report carries no numeric 'total'".to_string())? as usize;
    let raw = json
        .get("failures")
        .and_then(Json::as_array)
        .ok_or_else(|| "the report carries no 'failures' array".to_string())?;
    let mut failures = Vec::with_capacity(raw.len());
    for (index, failure) in raw.iter().enumerate() {
        let entry_name = failure
            .get("entry_name")
            .and_then(Json::as_str)
            .ok_or_else(|| format!("failure {index} carries no 'entry_name' string"))?;
        failures.push(Failure {
            entry_name: entry_name.to_string(),
            query: string_field(failure, "query"),
            detail: string_field(failure, "detail"),
            prometheus_body: failure
                .get("prometheus_body")
                .cloned()
                .unwrap_or(Json::Null),
            ravel_body: failure.get("ravel_body").cloned().unwrap_or(Json::Null),
        });
    }
    Ok(RunReport { total, failures })
}

/// An optional string field of an archived failure. Only `entry_name` decides
/// anything the report publishes; the rest is context for a reader.
fn string_field(failure: &Json, field: &str) -> String {
    failure
        .get(field)
        .and_then(Json::as_str)
        .unwrap_or_default()
...
/// state-2 rejection case verified. `differential` carries the agreed
/// dimension, and only `Some` populates it.
#[allow(clippy::expect_used)]
async fn build_report_with(differential: Option<&RunReport>) -> ConformanceReport {
    let parsed = parsed_corpus();
    ...
    if let Some(run) = differential {
        report
            .apply_run_report(run)
            .expect("the differential run report must cover the corpus");
    }
    report
}
