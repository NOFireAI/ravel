...
impl MetricsIngestReport {
    ...
    pub fn pooled_ack_p99_ms(&self, systems: &[&str]) -> Result<f64, AckConflation> {
        let rows: Vec<&SystemIngestResult> =
            systems.iter().filter_map(|s| self.row(s)).collect();
...
pub fn parse_logical_stream(text: &str) -> Vec<LogicalSample> {
    ...
    for line in text.lines() {
        ...
        let (Some(ts), Some(series), Some(payload)) =
...
pub async fn replay_into_ravel(
    ...
    for chunk in valid.chunks(batch_size) {
        ...
        let result = router
            .write_values(config.tenant.clone(), points, WriteMode::Strict, ack_deadline)
            .await;
        match result {
            Ok(receipt) => {
    ...
    let result = SystemIngestResult::ravel_row("in-process".to_string(), ingest, encoded_tokens, storage);
...
mod tests {
    ...
    fn strict_ack_is_reported_separately_from_buffered() {
        ...
        assert_ne!(ravel_ack, prom_ack, "the ack meanings must not collapse in JSON");
    }
    ...
    async fn replay_of_n_valid_samples_accepts_exactly_n_and_rejects_zero() {
        ...
        assert_eq!(ing.accepted_samples, n as u64, "every valid sample accepted");
        ...
        assert!(storage.object_count > 0, "the run wrote at least one object");
    ...
    fn parse_logical_stream_reads_float_lines_only() {
        ...
        assert_eq!(parsed.len(), 1, "only the one float line is a scalar sample");
