...
pub fn render(report: &MetricsBenchReport) -> Result<String, RenderError> {
    ...
    let _ = writeln!(
        ...
        p.backend.store_backend, p.backend.region, p.backend.endpoint, p.backend.backend_bills_requests
    );
    let _ = writeln!(
        out,
    ...
    for class in CostClass::ALL {
        let in_class: Vec<&Measurement> =
            report.measurements.iter().filter(|m| m.class == *class).collect();
    ...
    match report.geomean_ms {
        None => {
            out.push_str("  geomean: not computed (per-query rows above are the source of truth)\n");
        }
        Some(g) => {
            let _ = writeln!(
                out,
...
mod tests {
    ...
    fn a_duplicate_measurement_id_fails_validation() {
        let mut report = valid_report();
        report.measurements.push(timed_measurement("mb_fanout_total_rate"));
    ...
    fn an_ok_measurement_missing_a_timing_figure_fails_validation() {
        let mut report = valid_report();
        report.measurements[0].figures.retain(|f| f.name != "median_ms");
    ...
    fn the_renderer_derives_the_table_and_carries_the_retry_caveat() {
        ...
        assert!(text.contains("12.500"), "the median figure is rendered: {text}");
        // The retry caveat is in the output.
        assert!(text.contains(RETRY_CAVEAT), "the retry caveat must be rendered: {text}");
