...
enum Backend {
    ...
    // executor; kept so the same harness runs on target hardware.) Boxed:
    // S3Config is much larger than the InProcess variant, and clippy's
    // large-enum-variant lint fires on the unboxed spread.
