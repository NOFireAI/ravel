...
//! wired and un-wired, crossed with ADR-0107's whole-object and ranged read
//! shapes (issue #693) -- so the report shows the scan fanning out past the
//! segment count, the object-store GET count on each shape, and the
//! `reads/sg` and `bytes_x` columns that turn the un-cached repeats into a
//! single amplification factor.
//!
...
struct Args {
    ...
    /// rows measure a whole-object read under a ranged label.
