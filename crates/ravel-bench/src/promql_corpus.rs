...
mod tests {
    ...
    fn the_registry_check_admits_a_construct_ravel_refuses() {
        ...
        gate_corpus(&[refused]).expect("a registered but unsupported construct stays in the corpus");
    }
    ...
    fn a_malformed_corpus_is_a_typed_parse_error() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = write_corpus(&dir, "corpus.json", "{\"version\": 1, \"entries\": [{\"id\":");
        ...
        assert!(
            err.to_string()
                .contains("not a valid JSON corpus document")
        );
    }
    ...
    fn the_loader_gates_what_it_returns() {
        ...
        assert!(entries[0].runs_under("ci"), "an empty profile list is every profile");
        assert!(
            entries[1]
                .modified
                .reason()
