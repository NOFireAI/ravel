...
fn render_report(dir: &tempfile::TempDir, name: &str, report: &MetricsBenchReport) -> Output {
    ...
    std::fs::write(&path, serde_json::to_string_pretty(report).expect("serialize")).expect("write");
    Command::new(bin())
...
fn a_valid_report_renders_through_the_binary_with_the_retry_caveat() {
    ...
    assert!(stdout.contains("12.500"), "the median figure is rendered: {stdout}");
    ...
    assert!(stdout.contains("SKIP"), "an unregistered band must render a loud SKIP: {stdout}");
}
...
fn a_report_missing_a_provenance_field_fails_through_the_binary() {
    ...
    assert!(!out.status.success(), "a blank git commit must fail through the binary");
...
fn a_non_finite_figure_fails_through_the_binary() {
    ...
    // serde_json renders NaN as `null` by default, which fails to deserialize
    // back into an f64 field, so write the malformed value explicitly as a raw
    // JSON number to reach the finiteness check rather than the parser.
    let mut doc = serde_json::to_value(&report).expect("serialize");
    doc["measurements"][0]["figures"][1]["value"] =
        serde_json::from_str("1e999").expect("infinity literal parses to a JSON number");
    ...
    let out = Command::new(bin())
        ...
        .output()
        .expect("spawn");
    assert!(!out.status.success(), "a non-finite figure must fail through the binary");
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("not finite") || stderr.contains("median_ms"),
        "the binary must reject the non-finite figure: {stderr}"
    );
}
