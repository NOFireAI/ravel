...
fn the_ingest_lane_is_reachable_from_the_bin_and_ravel_is_durable_on_ack() {
    ...
    assert_eq!(systems.len(), 1, "only the in-process Ravel row without endpoints");
    let ravel = &systems[0];
    ...
    assert!(
        ravel["commit_tokens"].as_array().is_some_and(|t| !t.is_empty()),
        "the durable-on-ack row records at least one commit token"
    );
    assert!(ravel["storage"].is_object(), "the Ravel row carries storage accounting");
