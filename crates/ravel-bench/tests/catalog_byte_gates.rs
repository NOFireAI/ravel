...
impl Breakdown {
    ...
    /// Every section plus the residual sum exactly to the object. This is the
    /// invariant the old accounting silently broke.
    fn assert_sums(&self) {
        let sum: u64 = self.sections.iter().map(|(_, l)| *l).sum::<u64>() + self.residual;
        assert_eq!(
            sum, self.object_bytes,
            "section bytes + residual must equal the object"
        );
    }
}
...
fn catalog_byte_gates_golden() {
    ...
    if std::env::var_os("REGEN_CATALOG_BYTE_GATES").is_some() {
