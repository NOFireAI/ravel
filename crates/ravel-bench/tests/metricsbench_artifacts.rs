...
fn the_family_shares_partition_every_profiles_active_series_exactly() {
    ...
    // histogram instance.
    assert_eq!(workload.series_per_instance(FamilyKind::ClassicHistogram), 10);
...
fn the_corpus_splits_exactly_six_range_queries_from_eighteen_instant_ones() {
    ...
    let range = entries
        .iter()
        .filter(|e| e.eval == EvalKind::Range)
        .count();
    let instant = entries
        .iter()
...
fn every_named_construct_is_in_the_promql_registry_and_the_corpus_names_a_known_number_of_them() {
    ...
    // The corpus is a workload sample, not a conformance suite: it names 33 of
    // the registry's constructs. Pinned exactly so adding an entry that names
    // nothing new, or dropping the only entry that names a construct, is
    // visible.
    assert_eq!(named.len(), 33);
    assert_eq!(known.len(), 128);
...
fn every_metric_and_label_literal_the_corpus_selects_exists_in_the_workload() {
    ...
    for entry in &entries {
        for word in entry.promql.split(|c: char| !c.is_ascii_alphanumeric() && c != '_') {
    ...
    assert_eq!(
        workload.generator.scaling_label_value_prefix,
        "metricsbench-instance-",
...
fn corrupt_corpus(dir: &tempfile::TempDir, name: &str, mutate: impl Fn(&mut serde_json::Value)) -> PathBuf {
...
fn a_corrupt_workload_manifest_fails_through_the_binary() {
    ...
    let mut doc: serde_json::Value = serde_json::from_str(&text).expect("the real manifest is JSON");
...
fn the_binary_refuses_an_unknown_profile_rather_than_defaulting() {
    ...
    let stderr = String::from_utf8_lossy(&out.stderr);
    assert!(
        stderr.contains("declares no profile `smoke`"),
        "{stderr}"
    );
