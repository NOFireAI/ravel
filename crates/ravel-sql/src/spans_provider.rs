...
impl TableProvider for SpansTableProvider {
    ...
    async fn scan(
        ...
        // `attrs` map, the `events` list, and the `links` list, and no pending
        // erasure predicate applies. Push the
        ...
        // Ineligible path, unchanged: the scan emits the full thirteen-column
        // schema and, for column selection, this `ProjectionExec` drops the
        // rest. The fetch reads the whole object regardless (an `attrs`,
        // `events`, or `links` projection, or a pending erasure predicate,
        // forces the row path, which
