...
/// Render the example-SQL manifest: one tab-separated line per enumerated
/// construct (`<category>\t<construct>\t<example SQL>`), sorted the same way as
/// the published table.
///
/// This is a SECOND generated artifact (`docs/sql-conformance-examples.txt`),
/// deliberately separate from [`render_document`]'s prose table. It records, as
/// an independently reviewable fact, the exact SQL each conformance row
/// executes. The prose document is regenerated by `REGEN_SQL_CONFORMANCE=1`;
/// this manifest is not, so regenerating the prose table to match a mutated
/// example cannot also launder the record of what that row actually runs
/// (#408). Its own check (`example_sql_matches_committed_manifest`) is
/// regenerated only by the distinct, deliberate `REGEN_SQL_CONFORMANCE_EXAMPLES`
/// action, which produces a reviewable diff of the changed SQL.
///
/// The `example` field is emitted verbatim; no construct's example contains a
/// tab or newline, so each record stays on one line and splits cleanly on `\t`.
pub fn render_example_manifest(constructs: &[Construct]) -> String {
    let mut rows: Vec<&Construct> = constructs.iter().collect();
    rows.sort_by(|a, b| {
        a.category
            .ordinal()
    ...
    let mut out = String::new();
    out.push_str(
        "# SQL conformance example manifest -- GENERATED, do not edit by hand.\n\
         #\n\
         # The exact SQL each conformance row executes, one record per line as\n\
         # \"<category>\\t<construct>\\t<example SQL>\". Regenerated ONLY by\n\
         #\n\
         #     REGEN_SQL_CONFORMANCE_EXAMPLES=1 cargo test -p ravel-sql --test conformance\n\
         #\n\
         # and never by the prose document's REGEN_SQL_CONFORMANCE=1. This keeps\n\
         # \"the registry changed\" and \"an example's SQL changed\" as two separate,\n\
         # independently reviewed facts: regenerating docs/sql-conformance.md\n\
         # cannot silently change what SQL a row runs (#408).\n\
         #\n",
    );
    for c in rows {
        let _ = writeln!(out, "{}\t{}\t{}", c.category.label(), c.name, c.example);
    }
    out
}
