...
pub fn build_session(
    ...
    // decisions 2, 4). This MUST run before the per-table UDF registration
    // below: `label`/`label_match`/`has_word` are admitted but not yet
    // registered here, so enumerating now sees only the upstream defaults and
    // cannot deregister Ravel's own UDFs. Running it after registration would
    // instead drop `has_word` from a logs session.
...
mod tests {
    ...
    /// Deliverable 2's ordering constraint: the scalar deregistration loop runs
    /// BEFORE per-table UDF registration, so a logs session keeps `has_word`
    /// and a metrics session keeps `label`/`label_match`. A wrong order would
    /// enumerate those names and deregister them, silently stripping the table
    /// its query needs.
    #[test]
