...
async fn selective_numeric_reads_only_surviving_blocks() {
    ...
    // Exact GET count, 7 per segment with the 64 MiB cache absorbing every
    // re-touch: the plan's suffix probe, its SKIP_IDX and FIELD_DIR range reads,
    // then the scan's footer, header, tail-directory and single surviving-block
    // reads. No whole-object GET anywhere. The scan's footer read is what the
    // carried footer costs: skipping the probe means the footer bytes are no
    // longer resident, so they are fetched as their own range instead. That
    // trade is request-neutral on a cold cache and one GET per segment worse on
    // a warm one, which is why this figure is 56 rather than the 48 the same
    // shape reads with the footer carry disabled. It is a property of the #693
    // part 3 carry, not of #761's pruning: the bytes are identical either way.
...
async fn selective_third_no_partition_multiplication_under_cache_pressure() {
    ...
    // The margin names the flip: with `resolve_extents` passing `&[]` again this
    // same shape moves 1,306,268 bytes under the same 32 KiB cache, twice the
    // 650,087 of a full scan, because each of the 8 partitions re-reads the
    // whole object it owns a block in. The bound above rules that out by more
    // than a factor of two, so it cannot pass on a tree where the blocks are
    // fetched whole and eviction happens to be mild. `small.full_gets == 0`
    // above closes the same door from the other side: no GET is ever a whole
    // object, so a partition cannot re-read one however hard the cache thrashes.
}
