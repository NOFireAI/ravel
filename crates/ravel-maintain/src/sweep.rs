...
impl BucketScan {
    async fn load(
        ...
        let entries = list_commit_entries_scoped(store, tenant, signal, shard, Some(&[hour])).await?;
        let rewrites = load_rewrite_records(store, &entries).await?;
...
async fn superseded_inputs_outstanding(
    ...
    for drop in &completion.bucket_drops {
        ...
        if !scans.contains_key(&cache_key) {
            let scan =
                BucketScan::load(store, tenant, signal, drop.shard, drop.ingest_hour_bucket).await?;
            scans.insert(cache_key, scan);
        }
