...
async fn attempts_equal_calls_with_no_faults() {
    ...
    store
        .delete("attempts/quiet")
        .await
        .expect("a healthy delete must succeed");

    let snap = metrics.snapshot();
    // No fault fired: exactly one request per op on the wire.
    assert_eq!(fake.count(Op::Get), 1, "a quiet get is one GET");
    assert_eq!(fake.count(Op::Delete), 1, "a quiet delete is one DELETE");
    assert_eq!(
        fake.requests(Op::Get)
            .iter()
            .filter(|seen| seen.fault.is_some())
    ...
    assert_eq!(snap.delete.attempts, 1);
    assert_eq!(snap.delete.calls, 1);
    assert_eq!(
        snap.delete.attempts, snap.delete.calls,
        "no retry: attempts equal calls on the delete"
    );
}
