...
/// immediately preceding version, so a retired version (RSPAN v1, v2) stays
...
impl SupportedVersions {
    ...
    /// one RSPAN version is readable (ADR-0054 kept no dual reader), so there
    /// is no N-1 to accept and the reader behaves identically to the old
    /// single-version gate.
    ...
    /// Used at the first format bump that ships a dual reader; no RSPAN version
    /// uses it today.
...
/// RSPAN's supported-version window. Today it resolves to the single current
...
mod tests {
    ...
    /// Today's RSPAN window resolves to exactly the single current version, so
    /// the reader's accepted set is byte-for-byte the pre-ADR-0066 behaviour:
    /// v3 accepted, v1/v2 and a hypothetical v4 rejected. Only the window's
    /// shape is new machinery.
    #[test]
    ...
    /// The N/N-1 window shape, proven on a synthetic version number (no RSPAN
    /// version below v3 has a byte fixture). The window is exactly two versions
