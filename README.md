...
following the quickstart. Every entry below is in the pinned `0.17.0` image;
`git log v0.17.0..main` is what tells you whether that is still true after the
...
quickstart pins `ghcr.io/nofireai/ravel-server:0.17.0`. Override it with
`RAVEL_IMAGE`.
...
```sh
cosign verify \
  --certificate-identity 'https://github.com/NOFireAI/ravel/.github/workflows/publish-images.yml@refs/tags/v0.17.0' \
  --certificate-oidc-issuer 'https://token.actions.githubusercontent.com' \
  ghcr.io/nofireai/ravel-server:0.17.0
```

Replace `v0.17.0` and `0.17.0` with the release you are verifying. The tag ref in
