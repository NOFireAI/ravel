...
following the quickstart. Every entry below is in the pinned `0.16.1` image;
`git log v0.16.1..main` is what tells you whether that is still true after the
...
quickstart pins `ghcr.io/nofireai/ravel-server:0.16.1`. Override it with
`RAVEL_IMAGE`.
...
```sh
cosign verify \
  --certificate-identity 'https://github.com/NOFireAI/ravel/.github/workflows/publish-images.yml@refs/tags/v0.16.1' \
  --certificate-oidc-issuer 'https://token.actions.githubusercontent.com' \
  ghcr.io/nofireai/ravel-server:0.16.1
```

Replace `v0.16.1` and `0.16.1` with the release you are verifying. The tag ref in
