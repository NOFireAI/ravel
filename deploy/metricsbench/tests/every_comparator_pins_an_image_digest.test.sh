...
# Categories 1-5 are fully pinned; category 6 fails on exactly the one
# curlimages/curl reference issue #1720's residual round left unpinned (see
# the comment block above). This is not a regression: it is the current,
# accurate state of the tree, and this case pins that state down so a
# category silently going red or green for an unrelated reason is caught.
d="$(new_tree committed)"
check "the committed tree fails only on the known-unpinned curlimages/curl k8s image" \
  "${d}" 1 "FAIL: 1 k8s manifest image reference(s) lack an @sha256: digest"
check "the committed tree's only unpinned k8s reference is curlimages/curl" \
  "${d}" 1 "floci.yaml:164: curlimages/curl:8.11.1"
...
# these checks assert exit 1 (not 0) only because category 6 separately fails
# on the one known-unpinned curlimages/curl k8s reference (see the "committed
# tree" case above), not because of anything these variable refs do.
