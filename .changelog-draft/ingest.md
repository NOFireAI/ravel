...
- **A structured OTLP log body (an array or a map) is now stored instead of
  ...
  key it compared; a body built to maximize that cost previously spent
  measurable seconds of CPU and tens of megabytes of allocation on a record
  the size limit was about to reject anyway.

- **PromQL binary operators (`+`, `-`, `*`, `/`, comparisons) now work
  between two native-histogram series, and OTLP rejects a classic-histogram
  shape Prometheus itself would reject** (issue #1700). Histogram schemas
  are reconciled to a common bucket layout before arithmetic runs, matching
  Prometheus' own reconciliation rules: buckets that straddle a zero
  threshold are kept rather than merged away, and a query over an
  unalignable pair of custom-bucket layouts now warns with the same
  bucket-layout text Prometheus uses instead of silently producing a wrong
  result. On the write path, an OTLP classic histogram whose bucket
  boundaries are non-finite, not ascending, or otherwise a shape Prometheus'
  own ingestion would refuse is now rejected at admission rather than stored
  and only failing later at query time.
...
- **A shard actor that dies and is respawned no longer silently halves
  ...
  of how far apart those outages are; the per-writer monotonic clock floor
  used to detect backwards clock steps is now carried across a respawn
...
- **The OTAP gRPC metrics surface gained the same histogram-explosion cap
  ...
  identically. Two related exemplar-accounting fixes shipped alongside it:
  OTAP was undercounting dropped histogram exemplars against a
  whole-request rejection (a request rejected for being over the exploded
  total, or over the wire count, discarded any exemplars it had already
  decoded without counting them as dropped, where the OTLP surface always
  counted them), and a third payload type, exponential-histogram data
  points, was missing from the dropped-exemplar count entirely on the path
  that rejects them as unsupported. All three OTAP payload types now
  report dropped exemplars with the same fidelity OTLP does.
