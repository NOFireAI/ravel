...
impl Running {
    ...
    pub async fn shutdown(self) -> anyhow::Result<()> {
        ...
        let outcome = tokio::time::timeout(shutdown_timeout, drain).await;
        let captured = listener_err_cell
            .lock()
            .ok()
            .and_then(|mut slot| slot.take());
        shutdown_outcome(outcome.is_err(), captured, shutdown_timeout)
    }
}
...
/// level rather than report a clean shutdown. A listener error captured before
/// the overrun is folded in as context rather than dropped; when the drain did
/// complete, a captured listener error is surfaced directly.
fn shutdown_outcome(
    timed_out: bool,
    ...
    if timed_out {
        ...
        return Err(match captured_listener_err {
            Some(listener_err) => {
                overrun.context(format!("listener error during shutdown: {listener_err}"))
            }
            None => overrun,
        });
    }
    match captured_listener_err {
        Some(err) => Err(err),
        None => Ok(()),
    }
}
...
mod shutdown_drain_tests {
    ...
    fn shutdown_outcome_ok_when_clean() {
        shutdown_outcome(false, None, Duration::from_secs(25))
    ...
    fn shutdown_outcome_overrun_is_an_error() {
        let err = shutdown_outcome(true, None, Duration::from_secs(25))
