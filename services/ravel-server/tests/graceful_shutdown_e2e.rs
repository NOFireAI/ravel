...
async fn audit_drain_is_bounded_by_the_overall_shutdown_timeout() {
    ...
    assert!(
        chain.contains("exceeded --shutdown-timeout"),
        "the overrun must name the budget that cut it off, got: {chain}"
    );
    assert!(
        elapsed >= SHUTDOWN_TIMEOUT && elapsed < SHUTDOWN_TIMEOUT * 5,
        "shutdown must return on the {SHUTDOWN_TIMEOUT:?} overall budget, not early and not on \
         the {AUDIT_MAX_AGE:?} audit bound; took {elapsed:?}"
    );
    assert_eq!(
