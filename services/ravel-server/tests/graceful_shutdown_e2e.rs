...
/// finished and the overrun says the records WERE flushed).
#[tokio::test]
...
async fn audit_drain_is_bounded_by_the_overall_shutdown_timeout() {
    ...
    // the "flush completed" branch: the message says the records WERE flushed,
    // in contrast to `shutdown_timeout_during_ingest_flush_warns_records_may_not_be_durable`.
    assert!(
        headline.contains("buffered records were flushed"),
        "an overrun after the flush completed must report the records flushed, got: {headline}"
    );
    assert!(
