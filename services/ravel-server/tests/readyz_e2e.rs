...
/// loop's first sleep, so once this call returns `0` means only "no probe
/// task has been spawned in this process" (the pre-spawn startup gap and a
/// pre-1970 clock are the two other, non-paging sources of `0`; see
/// `store_probe`'s doc comment on `PROBE_LAST_RUN_UNIX_NS`). Without that
/// stamp the gauge holds `0` for a whole jittered
